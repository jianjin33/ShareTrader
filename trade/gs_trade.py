...
class GSATrade(object):
    ...
    # 根据可用资金计算开仓数量
    def calculate_volumn(self, price: int):
        return int(self._cash / price / 100) * 100

    # 统计每单交易收益
    def stat_pnl(self, order: Order):
        buy_amount = order.open_price * order.volumn
        sell_amount = order.close_price * order.volumn
    ...
    def buy(self, params: TradeParams):
        if self.exist_order() == False:
            ...
            order.volumn = self.calculate_volumn(params.price)

            self._current_orders[key] = order
            self._cash -= order.volumn * order.open_price
            print('buy price:{0},volumn:{1},cash:{2}'.format(
                order.open_price, order.volumn, self._cash))
    ...
    def sell(self, params: TradeParams):
        if self.exist_order():
            ...
            self._cash += order.volumn * order.close_price
            self.stat_pnl(order)
            self._history_orders[key] = self._current_orders.pop(key)
            print('sell price:{0},pnl:{1},volumn:{2},cash:{3}'.format(
                order.close_price, order.pnl, order.volumn, self._cash))

    def get_hisotry_orders(self):
